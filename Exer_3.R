...
updated_data = na.omit(data_import) %>%
  ...
  avg_income_by_income_category=updated_data %>%
  group_by(income_category) %>%
  summarize(avg_income = mean(Total_income))

  avg_income_by_age_category=updated_data %>%
  group_by(age_category) %>%
  summarize(avg_incomeage = mean(Total_income))


  avg_income_by_family_status=updated_data %>%
